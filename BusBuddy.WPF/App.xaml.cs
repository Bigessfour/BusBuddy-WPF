...
public partial class App : Application
{
    ...
    /// <summary>
    /// Consolidated resource dictionary validation logic
    /// </summary>
    private void ValidateResources()
    {
        ...
        {
            Log.Error(ex, "❌ [STARTUP] Resource dictionary validation failed");
        }
    }

    /// <summary>
    /// Consolidated theme status confirmation logging
    /// </summary>
    private void ConfirmThemeStatus()
    {
        try
        {
        ...
        using (LogContext.PushProperty("Operation", "ParallelStartup"))
        {
            ...
            try
            {
                ...
                var startupTasks = new List<Task>
                {
                    ...
                    {
                        await Task.Run(() => ValidateResources());
                        return Task.CompletedTask;
                    }),
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            _startupStopwatch.Stop();
            BusBuddy.WPF.Utilities.PerformanceOptimizer.StopTiming("ApplicationStartup");
            Logger.Fatal(ex, "❌ Application startup failed after {ElapsedMs}ms", _startupStopwatch.ElapsedMilliseconds);
    ...
    private void StartEnhancedUIInitialization(StartupEventArgs e)
    {
        using (LogContext.PushProperty("Operation", "UIInitialization"))
        {
            ...
            try
            {
                ...
                // THEME CONFIRMATION PHASE - Now that the host is started, Serilog should be initialized
    ...
    private void ExecuteUIInitializationPhases()
    {
        ...
        try
        {
            Log.Information("🎨 [STARTUP] UI initialization phases started");
            // Resource validation now handled in ExecuteParallelStartupAsync via ValidateResources()
        }
        catch (Exception resourceEx)
        {
            Log.Error(resourceEx, "❌ [STARTUP] Error during UI initialization phases");
        ...
        {
            uiStopwatch.Stop();
            Log.Information("🎨 [STARTUP] UI initialization phases completed in {ElapsedMs}ms", uiStopwatch.ElapsedMilliseconds);
        }
    }
...
// Create marker file to indicate MainWindow is shown
...
_ = Task.Run(async () =>
{
    ...
    finally
    {
        ...
        Log.Information("[STARTUP] Orchestration timing completed in {ElapsedMs}ms", orchestrationStopwatch.ElapsedMilliseconds);
    }
});
...
#else
        Log.Information("[STARTUP] Log consolidation is only available in DEBUG builds");
#endif
    }
    catch (Exception ex)
    {
...
using (LogContext.PushProperty("Operation", "StartupCompletion"))
{
    ...
    Log.Information("⏱️ [STARTUP] Total startup time: {ElapsedMs}ms", _startupStopwatch.ElapsedMilliseconds);
            ...
            using (LogContext.PushProperty("Operation", "StartupFailure"))
{
    ...
    Log.Fatal(ex, "❌ [STARTUP] Fatal error during OnStartup method execution after {ElapsedMs}ms",
             _startupStopwatch?.ElapsedMilliseconds ?? 0);
        ...
        foreach (var arg in args)
        {
            switch (arg.ToLower())
            {
                ...
                default:
                    ...
                    if (arg.StartsWith("--"))
                    {
                        Log.Warning("Debug command line arguments are only available in DEBUG builds: {Argument}", arg);
                    }
...
private void ConfigureUtilities(IServiceCollection services)
{
...
#if DEBUG
    // Register log consolidation utility — only available in DEBUG builds
    services.AddScoped<BusBuddy.WPF.Utilities.LogConsolidationUtility>(provider =>
    {
        ...
        return new BusBuddy.WPF.Utilities.LogConsolidationUtility(logsDirectory);
    });
#endif
