    ...
    public partial class EnhancedDashboardView : UserControl
    {
        ...
        public EnhancedDashboardView()
        {
            try
            {
                ...
                // Initialize XAML error handling first
                BusBuddy.WPF.Utilities.XamlErrorHandler.Initialize();

                // Verify theme resources are available before InitializeComponent
                if (!VerifyThemeResources())
                {
                    _logger?.LogWarning("⚠️ EnhancedDashboardView: Theme resources verification failed, attempting fallback");
                    ApplyFallbackTheme();
                }
                ...
                // The InitializeComponent method is generated by the XAML compiler and should be available.
                // If you still get an error, clean and rebuild the solution to regenerate the .g.cs files.
                InitializeComponent();
            ...
            catch (XamlParseException xamlEx)
            {
                ...
                // Try to apply fallback theme and reinitialize
                try
                {
                    ApplyFallbackTheme();
                    InitializeComponent();
                    _logger?.LogInformation("✅ EnhancedDashboardView: Fallback theme applied successfully");
                }
                catch (Exception fallbackEx)
                {
                    _logger?.LogError(fallbackEx, "❌ EnhancedDashboardView fallback initialization failed: {ErrorMessage}", fallbackEx.Message);
                    throw new InvalidOperationException("Failed to initialize EnhancedDashboardView with both primary and fallback themes", fallbackEx);
                }
            }
            catch (Exception ex)
            {
